...
PS.init = function( system, options ) {
    ...
    PS.statusText("Goal Warp: Level 1")
};
...
const moveUp = function() {
    if (PS.color(playerX, playerY - 1) === PS.COLOR_WHITE) {
        ...
        mostRecentMove = "up"
        const [x, y] = findTeleporter(playerX, playerY, mostRecentMove)
    ...
    if (PS.color(winX, winY - 1) === PS.COLOR_WHITE) {
        ...
        mostRecentWinMove = "up"
        const [x, y] = findTeleporter(winX, winY, mostRecentWinMove)
...
const touchGoal = function() {
    if (levelNumber === 1) {
        reset()
        PS.color(20, 20, PS.COLOR_BLACK)
        PS.color(20, 12, PS.COLOR_BLACK)
        PS.color(20, 4, PS.COLOR_BLACK)
        PS.color(12, 20, PS.COLOR_BLACK)
        PS.color(4, 20, PS.COLOR_BLACK)
        PS.statusText("Goal Warp: Level 2")
    ...
    if (levelNumber === 2) {
        reset()
        PS.color(20, 20, PS.COLOR_WHITE)
        PS.color(20, 12, PS.COLOR_WHITE)
        PS.color(20, 4, PS.COLOR_WHITE)
        PS.color(12, 20, PS.COLOR_WHITE)
        PS.color(4, 20, PS.COLOR_WHITE)
        for (let x = 9; x <= 15; x++) {
            for (let y = 9; y <= 15; y++) {
                PS.color(x, y, PS.COLOR_BLACK)
            }
        }
        PS.statusText("Goal Warp: Level 3")
        PS.timerStop(hintTimerID)
        hintTimerID = PS.timerStart(1800, hint)
        levelNumber++
        return
...
const findTeleporter = function(startX, startY, recentMove) {
    if (recentMove === "down") {
        let x = startX
        let y = startY + 2
